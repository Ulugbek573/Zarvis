...
export default function Workers() {
  ...
  const handleSaveEdit = () => {
    setWorkers(
      workers.map((c) => (c.id === editing.id ? editing : c))
    );
    setEditing(null);
  };
  ...
  const handleSaveAdd = (data) => {
    setWorkers([...workers, { ...data, id: Date.now() }]);
    setAdding(false);
    setPage(Math.ceil((workers.length + 1) / PER_PAGE));
  };
  ...
  return (
    <>
      ...
      <main className="dashboard-page workers-page">
        {/* HEADER */}
        <div className="workers-header">
          <div>
            <h2>Mijozlar</h2>
            <span className="count">{workers.length} Users</span>
          </div>
