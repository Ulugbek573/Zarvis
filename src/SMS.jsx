...
export default function SMS() {
  const [sms, SMS] = useState(initialSMS);
  ...
  const handleDelete = (id) => {
    if (window.confirm("Mijozni o‘chirmoqchimisiz?")) {
      setSMS(sms.filter((c) => c.id !== id));
    }
  };

  const handleSaveEdit = () => {
    setSMS(
      sms.map((c) => (c.id === editing.id ? editing : c))
    );
    setEditing(null);
  };

  const handleSaveAdd = (data) => {
    setSMS([...sms, { ...data, id: Date.now() }]);
    setAdding(false);
    setPage(Math.ceil((MdSms.length + 1) / PER_PAGE));
  };
  ...
  return (
    <>
      ...
      <main className="dashboard-page sms-page">
        <div className="sms-header">
          <div>
            <h2>Mijozlar</h2>
            <span className="count">{sms.length} Users</span>
          </div>
