...
export default function Notifications() {
  ...
  const handleDelete = (id) => {
    if (window.confirm("Mijozni o‘chirmoqchimisiz?")) {
      setNotifications(notifications.filter((c) => c.id !== id));
    }
  };

  const handleSaveEdit = () => {
    setNotifications(
      notifications.map((c) => (c.id === editing.id ? editing : c))
    );
    setEditing(null);
  };
  ...
  const handleSaveAdd = (data) => {
    ...
    setAdding(false);
    setPage(Math.ceil((notifications.length + 1) / PER_PAGE));
  };
    ...
    <>
      ...
      <main className="dashboard-page notifications-page">
        <div className="notifications-header">
          <div>
            <h2>Mijozlar</h2>
            <span className="count">{notifications.length} Users</span>
          </div>
...
function notificationsModal({ title, data, onClose, onSave }) {
